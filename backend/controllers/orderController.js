...
export const placeOrder = async (req, res) => {
  ...
  try {
    ...
    const paymentData = {
      ...
      amount: Math.round(newOrder.amount * 100), // Convert to smallest currency unit (e.g., cents)
      orderCode: `ORDER-${newOrder._id}`,
      description: `Thanh toán cho đơn hàng ${newOrder._id}`,
    ...
    try {
      ...
      throw new Error(`Lỗi khi tạo liên kết thanh toán: ${paymentError.message}`);
    }

    if (paymentResponse && paymentResponse.data && paymentResponse.data.checkoutUrl) {
